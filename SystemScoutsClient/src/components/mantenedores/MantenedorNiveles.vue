  ...
  <div class="mantenedor-section">
    ...
    <div class="table-container">
      <ModernMainScrollbar>
        <table class="data-table">
          <thead><tr><th>DESCRIPCIÓN</th><th>ORDEN</th><th>ESTADO</th><th class="text-center">ACCIONES</th></tr></thead>
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              ...
              <td class="actions-cell" data-label="Acciones">
                <div class="action-buttons">
                  ...
                  <button v-if="can.modificar" class="action-btn btn-edit" @click="editarItem(item)"><AppIcons name="edit" :size="16" /></button>
                  <button v-if="can.eliminar" class="action-btn" :class="item.vigente ? 'btn-delete' : 'btn-activate'" @click="item.vigente ? confirmarAnular(item) : confirmarActivar(item)"><AppIcons :name="item.vigente ? 'trash' : 'check'" :size="16" /></button>
                </div>
              </td>
            </tr>
            <tr v-if="filteredItems.length === 0"><td colspan="4" class="no-data">No se encontraron niveles</td></tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
    </div>
    <div v-if="modalVisible" class="modal-overlay" @click="cerrarModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>{{ editando ? 'EDITAR' : 'NUEVO' }} NIVEL</h3><button class="modal-close" @click="cerrarModal">×</button></div>
        <div class="modal-body"><form @submit.prevent="guardar">
          <div class="form-group"><label class="form-label">DESCRIPCIÓN:</label><input type="text" class="form-control" v-model="form.descripcion" @input="form.descripcion = form.descripcion.toUpperCase()" placeholder="EJ: NIVEL BÁSICO" required></div>
          <div class="form-group"><label class="form-label">ORDEN:</label><input type="number" class="form-control" v-model="form.orden" placeholder="EJ: 1" required></div>
    ...
    <div v-if="viewModalVisible" class="modal-overlay" @click="cerrarViewModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>👁 DETALLE NIVEL</h3><button class="modal-close" @click="cerrarViewModal">×</button></div>
        <div class="modal-body">
          ...
          <div class="view-group"><label class="view-label">ORDEN:</label><div class="view-value">{{ itemSeleccionado?.orden }}</div></div>
    ...
    <div v-if="cargando" class="loading-overlay"><div class="loading-content"><div class="spinner"></div><p>Cargando niveles...</p></div></div>
  </div>
</template>
<script setup>
...
function abrirModalCrear() {
  ...
  Object.assign(form, { id: null, descripcion: '', orden: 1, vigente: true })
  modalVisible.value = true
}
defineExpose({ abrirModalCrear })
const items = ref([]); const search = ref(''); const tempSearch = ref(''); const cargando = ref(false); const saving = ref(false)
const ejecutarBusqueda = () => { search.value = tempSearch.value }
const modalVisible = ref(false); const editando = ref(false); const viewModalVisible = ref(false); const itemSeleccionado = ref(null)
const form = reactive({ id: null, descripcion: '', orden: 1, vigente: true })
const cargarDatos = async () => {
  cargando.value = true
  try {
    const resp = await mantenedoresService.nivel.list()
    const raw = Array.isArray(resp) ? resp : (resp.results || resp.data || [])
    items.value = raw.map(n => ({ id: n.niv_id ?? n.NIV_ID ?? n.id, descripcion: (n.niv_descripcion ?? n.NIV_DESCRIPCION ?? n.descripcion ?? '').toString(), orden: n.niv_orden ?? n.NIV_ORDEN ?? n.orden ?? 0, vigente: !!(n.niv_vigente ?? n.NIV_VIGENTE ?? n.vigente ?? true) }))
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al cargar niveles' }) } finally { cargando.value = false }
}
...
const editarItem = (i) => { editando.value = true; Object.assign(form, { id: i.id, descripcion: i.descripcion, orden: i.orden, vigente: i.vigente }); modalVisible.value = true }
...
const guardar = async () => {
  saving.value = true
  try {
    const p = { niv_descripcion: form.descripcion, niv_orden: form.orden, niv_vigente: form.vigente }
    if (editando.value) { await mantenedoresService.nivel.partialUpdate(form.id, p); emit('show-message', { type: 'success', text: 'Nivel actualizado' }) }
    else { await mantenedoresService.nivel.create(p); emit('show-message', { type: 'success', text: 'Nivel creado' }) }
    cerrarModal(); await cargarDatos()
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al guardar nivel' }) } finally { saving.value = false }
}
const confirmarAnular = (i) => { emit('confirm-action', { titulo: 'Anular Nivel', mensaje: `¿Anular "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.nivel.partialUpdate(i.id, { niv_vigente: false }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Nivel anulado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
const confirmarActivar = (i) => { emit('confirm-action', { titulo: 'Activar Nivel', mensaje: `¿Activar "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.nivel.partialUpdate(i.id, { niv_vigente: true }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Nivel activado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
...
.data-table { width: 100%; border-collapse: collapse; } .data-table th, .data-table td {
  padding: 4px 15px;
  text-align: center;
