<template>
  <div class="mantenedor-section">
    <div class="mantenedor-header">
      ...
      <!-- <button class="btn-primary" @click="abrirModalCrear"><AppIcons name="plus" :size="18" /> Nuevo Estado Civil</button> -->
    </div>
      ...
      <ModernMainScrollbar>
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="filteredItems.length === 0"><td colspan="3" class="no-data">No se encontraron estados civiles</td></tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
    </div>
    <div v-if="modalVisible" class="modal-overlay" @click="cerrarModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>{{ editando ? 'EDITAR' : 'NUEVO' }} ESTADO CIVIL</h3><button class="modal-close" @click="cerrarModal">×</button></div>
        <div class="modal-body"><form @submit.prevent="guardar">
          <div class="form-group"><label class="form-label">DESCRIPCIÓN:</label><input type="text" class="form-control" v-model="form.descripcion" @input="form.descripcion = form.descripcion.toUpperCase()" placeholder="EJ: SOLTERO/A" required></div>
    ...
    <div v-if="viewModalVisible" class="modal-overlay" @click="cerrarViewModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>👁 DETALLE ESTADO CIVIL</h3><button class="modal-close" @click="cerrarViewModal">×</button></div>
    ...
    <div v-if="cargando" class="loading-overlay"><div class="loading-content"><div class="spinner"></div><p>Cargando...</p></div></div>
  </div>
</template>
<script setup>
...
const cargarDatos = async () => {
  ...
  try {
    ...
    items.value = raw.map(e => ({ id: e.esc_id ?? e.ESC_ID ?? e.id, descripcion: (e.esc_descripcion ?? e.ESC_DESCRIPCION ?? e.descripcion ?? '').toString(), vigente: !!(e.esc_vigente ?? e.ESC_VIGENTE ?? e.vigente ?? true) }))
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al cargar estados civiles' }) } finally { cargando.value = false }
}
...
const editarItem = (i) => { editando.value = true; Object.assign(form, { id: i.id, descripcion: i.descripcion, vigente: i.vigente }); modalVisible.value = true }
...
const guardar = async () => {
  saving.value = true
  try {
    const p = { esc_descripcion: form.descripcion, esc_vigente: form.vigente }
    if (editando.value) { await mantenedoresService.estadoCivil.partialUpdate(form.id, p); emit('show-message', { type: 'success', text: 'Estado civil actualizado' }) }
    else { await mantenedoresService.estadoCivil.create(p); emit('show-message', { type: 'success', text: 'Estado civil creado' }) }
    cerrarModal(); await cargarDatos()
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al guardar' }) } finally { saving.value = false }
}
const confirmarAnular = (i) => { emit('confirm-action', { titulo: 'Anular Estado Civil', mensaje: `¿Anular "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.estadoCivil.partialUpdate(i.id, { esc_vigente: false }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Anulado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
const confirmarActivar = (i) => { emit('confirm-action', { titulo: 'Activar Estado Civil', mensaje: `¿Activar "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.estadoCivil.partialUpdate(i.id, { esc_vigente: true }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Activado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
