<template>
  <div class="mantenedor-section">
    <div class="mantenedor-header">
      <h2><AppIcons name="book" :size="24" /> Gestión de Tipos de Curso</h2>
      <button class="btn-primary" @click="abrirModalCrear">
        <AppIcons name="plus" :size="18" /> Nuevo Tipo Curso
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="filteredItems.length === 0">
              <td colspan="5" class="no-data">No se encontraron tipos de curso</td>
            </tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
          ...
          <form @submit.prevent="guardar">
            ...
            <div class="form-group">
              ...
              <input 
                type="number" 
                class="form-control" 
                v-model.number="form.cant_participante"
                placeholder="EJ: 30"
              >
            </div>
...
const cargarDatos = async () => {
  ...
  try {
    ...
    showToast('Error al cargar tipos de curso', 'alert-triangle')
  } finally {
...
const abrirModalCrear = () => {
  ...
  Object.assign(form, { id: null, descripcion: '', tipo: 1, cant_participante: null, vigente: true })
  modalVisible.value = true
}

const editarElemento = (item) => {
  editando.value = true
  Object.assign(form, { id: item.id, descripcion: item.descripcion, tipo: item.tipo, cant_participante: item.cant_participante, vigente: item.vigente })
  modalVisible.value = true
}
...
const guardar = async () => {
  ...
  try {
    ...
    if (editando.value) {
      ...
      showToast('Tipo de curso actualizado correctamente', 'check')
    } else {
      await mantenedoresService.tipoCursos.create(payload)
      showToast('Tipo de curso creado correctamente', 'check')
    }
    cerrarModal()
    await cargarDatos()
  } catch (error) {
    console.error('Error guardando tipo de curso:', error)
    showToast('Error al guardar tipo de curso', 'alert-triangle')
  } finally {
    saving.value = false
  }
}
...
const confirmarAnular = (item) => {
  emit('confirm-action', {
    titulo: 'Anular Tipo de Curso',
    mensaje: `¿Estás seguro de anular "${item.descripcion}"?`,
    accion: async () => {
      try {
        await mantenedoresService.tipoCursos.partialUpdate(item.id, { tcu_vigente: false })
        await cargarDatos()
        showToast('Tipo de curso anulado correctamente', 'check')
...
const confirmarActivar = (item) => {
  emit('confirm-action', {
    titulo: 'Activar Tipo de Curso',
    mensaje: `¿Estás seguro de activar "${item.descripcion}"?`,
    accion: async () => {
      try {
        await mantenedoresService.tipoCursos.partialUpdate(item.id, { tcu_vigente: true })
        await cargarDatos()
        showToast('Tipo de curso activado correctamente', 'check')
