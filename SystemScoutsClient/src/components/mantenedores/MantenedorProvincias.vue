<template>
  <div class="mantenedor-section">
    <div class="mantenedor-header">
      <h2><AppIcons name="map-pin" :size="24" /> Gestión de Provincias</h2>
    ...
    <div class="search-bar">
      <input type="text" class="search-input" v-model="search" placeholder="Buscar Provincia...">
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              ...
              <td>{{ getRegionNombre(item.region_id) }}</td>
              <td>
            ...
            <tr v-if="filteredItems.length === 0"><td colspan="4" class="no-data">No se encontraron provincias</td></tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
    </div>
          ...
          <form @submit.prevent="guardar">
            <div class="form-group">
              ...
              <input type="text" class="form-control" v-model="form.descripcion"
                @input="form.descripcion = form.descripcion.toUpperCase()" placeholder="EJ: CONCEPCIÓN" required>
            </div>
            <div class="form-group">
              <label class="form-label">REGIÓN:</label>
              <select class="form-control" v-model="form.region_id" required>
                <option value="">SELECCIONE UNA REGIÓN</option>
                <option v-for="r in regiones" :key="r.id" :value="r.id">{{ r.descripcion }}</option>
              </select>
            </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="view-container">
            ...
            <div class="view-group"><label class="view-label">REGIÓN:</label><div class="view-value">{{ getRegionNombre(itemSeleccionado?.region_id) }}</div></div>
            <div class="view-group"><label class="view-label">ESTADO:</label><div class="view-value">
              <span class="status-badge" :class="itemSeleccionado?.vigente ? 'status-active' : 'status-inactive'">{{ itemSeleccionado?.vigente ? 'ACTIVO' : 'INACTIVO' }}</span>
            </div></div>
          </div>
    ...
    <div v-if="cargando" class="loading-overlay"><div class="loading-content"><div class="spinner"></div><p>Cargando provincias...</p></div></div>
  </div>
</template>
...
const form = reactive({ id: null, descripcion: '', region_id: null, vigente: true })

const cargarDatos = async () => {
  cargando.value = true
  try {
    const [respProv, respReg] = await Promise.all([
    ...
    emit('show-message', { type: 'error', text: 'Error al cargar provincias' })
  } finally { cargando.value = false }
}

const getRegionNombre = (id) => regiones.value.find(r => r.id === id)?.descripcion || 'NO ENCONTRADA'

const filteredItems = computed(() => {
  if (!search.value) return items.value
  const term = search.value.toLowerCase()
  return items.value.filter(p => p.descripcion.toLowerCase().includes(term))
})

const abrirModalCrear = () => { editando.value = false; Object.assign(form, { id: null, descripcion: '', region_id: null, vigente: true }); modalVisible.value = true }
const editarItem = (item) => { editando.value = true; Object.assign(form, { id: item.id, descripcion: item.descripcion, region_id: item.region_id, vigente: item.vigente }); modalVisible.value = true }
...
const guardar = async () => {
  saving.value = true
  try {
    const payload = { pro_descripcion: form.descripcion, reg_id: form.region_id, pro_vigente: form.vigente }
    if (editando.value) {
      await mantenedoresService.provincia.partialUpdate(form.id, payload)
      emit('show-message', { type: 'success', text: 'Provincia actualizada correctamente' })
    } else {
      await mantenedoresService.provincia.create(payload)
      emit('show-message', { type: 'success', text: 'Provincia creada correctamente' })
    }
    cerrarModal(); await cargarDatos()
  } catch (error) {
    console.error('Error guardando provincia:', error)
    emit('show-message', { type: 'error', text: 'Error al guardar provincia' })
  } finally { saving.value = false }
}

const confirmarAnular = (item) => {
  emit('confirm-action', { titulo: 'Anular Provincia', mensaje: `¿Estás seguro de anular la provincia "${item.descripcion}"?`,
    accion: async () => { try { await mantenedoresService.provincia.partialUpdate(item.id, { pro_vigente: false }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Provincia anulada' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error al anular provincia' }) } }
  })
}
const confirmarActivar = (item) => {
  emit('confirm-action', { titulo: 'Activar Provincia', mensaje: `¿Estás seguro de activar la provincia "${item.descripcion}"?`,
    accion: async () => { try { await mantenedoresService.provincia.partialUpdate(item.id, { pro_vigente: true }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Provincia activada' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error al activar provincia' }) } }
  })
}
