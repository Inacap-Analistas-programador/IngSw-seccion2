  ...
  <div class="mantenedor-section">
      ...
      <ModernMainScrollbar>
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="filteredItems.length === 0"><td colspan="4" class="no-data">No se encontraron conceptos</td></tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
    </div>
    <div v-if="modalVisible" class="modal-overlay" @click="cerrarModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>{{ editando ? 'EDITAR' : 'NUEVO' }} CONCEPTO CONTABLE</h3><button class="modal-close" @click="cerrarModal">×</button></div>
        <div class="modal-body"><form @submit.prevent="guardar">
          <div class="form-group"><label class="form-label">DESCRIPCIÓN:</label><input type="text" class="form-control" v-model="form.descripcion" @input="form.descripcion = form.descripcion.toUpperCase()" placeholder="EJ: MATRÍCULA" required></div>
          <div class="form-group"><label class="form-label">TIPO:</label>
            <select class="form-control" v-model="form.tipo" required>
              <option value="">SELECCIONE UN TIPO</option><option value="INGRESO">INGRESO</option><option value="EGRESO">EGRESO</option>
            </select>
          </div>
    ...
    <div v-if="viewModalVisible" class="modal-overlay" @click="cerrarViewModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header"><h3>👁 DETALLE CONCEPTO</h3><button class="modal-close" @click="cerrarViewModal">×</button></div>
        <div class="modal-body">
          ...
          <div class="view-group"><label class="view-label">TIPO:</label><div class="view-value">{{ itemSeleccionado?.tipo }}</div></div>
    ...
    <div v-if="cargando" class="loading-overlay"><div class="loading-content"><div class="spinner"></div><p>Cargando conceptos...</p></div></div>
  </div>
</template>
<script setup>
...
const form = reactive({ id: null, descripcion: '', tipo: '', vigente: true })
const cargarDatos = async () => {
  cargando.value = true
  try {
    const resp = await mantenedoresService.conceptoContable.list()
    const raw = Array.isArray(resp) ? resp : (resp.results || resp.data || [])
    items.value = raw.map(c => ({ id: c.coc_id ?? c.COC_ID ?? c.id, descripcion: (c.coc_descripcion ?? c.COC_DESCRIPCION ?? c.descripcion ?? '').toString(), tipo: c.coc_tipo ?? c.COC_TIPO ?? c.tipo ?? '', vigente: !!(c.coc_vigente ?? c.COC_VIGENTE ?? c.vigente ?? true) }))
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al cargar conceptos' }) } finally { cargando.value = false }
}
...
function abrirModalCrear() {
  ...
  Object.assign(form, { id: null, descripcion: '', tipo: '', vigente: true })
...
const guardar = async () => {
  saving.value = true
  try {
    const p = { coc_descripcion: form.descripcion, coc_tipo: form.tipo, coc_vigente: form.vigente }
    if (editando.value) { await mantenedoresService.conceptoContable.partialUpdate(form.id, p); emit('show-message', { type: 'success', text: 'Concepto actualizado' }) }
    else { await mantenedoresService.conceptoContable.create(p); emit('show-message', { type: 'success', text: 'Concepto creado' }) }
    cerrarModal(); await cargarDatos()
  } catch (e) { emit('show-message', { type: 'error', text: 'Error al guardar' }) } finally { saving.value = false }
}
const confirmarAnular = (i) => { emit('confirm-action', { titulo: 'Anular Concepto', mensaje: `¿Anular "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.conceptoContable.partialUpdate(i.id, { coc_vigente: false }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Anulado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
const confirmarActivar = (i) => { emit('confirm-action', { titulo: 'Activar Concepto', mensaje: `¿Activar "${i.descripcion}"?`, accion: async () => { try { await mantenedoresService.conceptoContable.partialUpdate(i.id, { coc_vigente: true }); await cargarDatos(); emit('show-message', { type: 'success', text: 'Activado' }) } catch (e) { emit('show-message', { type: 'error', text: 'Error' }) } } }) }
