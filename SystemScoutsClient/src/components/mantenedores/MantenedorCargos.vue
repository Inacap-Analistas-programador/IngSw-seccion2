  ...
  setup(props, { emit }) {
    ...
    const cargarDatos = async () => {
      ...
      try {
        ...
        items.value = normalize(getData(resp), c => ({
          id: c.car_id ?? c.CAR_ID ?? c.id,
          descripcion: (c.car_descripcion ?? c.CAR_DESCRIPCION ?? c.DESCRIPCION ?? c.descripcion ?? '').toString(),
          vigente: !!(c.car_vigente ?? c.CAR_VIGENTE ?? c.vigente)
        }))
      } catch (err) {
        console.error('Error cargando cargos:', err)
        emit('show-message', { type: 'error', text: 'Error al cargar cargos' })
      } finally {
        loading.value = false
      }
    }
    ...
    const guardar = async () => {
      ...
      try {
        const payload = {
          car_descripcion: form.descripcion,
          car_vigente: !!form.vigente
        }
        ...
        cerrarModal()
        await cargarDatos()
        emit('show-message', { type: 'success', text: editando.value ? 'Cargo actualizado' : 'Cargo creado' })
      } catch (err) {
    ...
    const confirmarAccion = (item, accion) => {
      emit('confirm-action', {
        ...
        accion: async () => {
          try {
            const payload = { car_vigente: accion === 'activar' }
            await mantenedoresService.cargo.partialUpdate(item.id, payload)
            await cargarDatos()
            emit('show-message', { type: 'success', text: `Registro ${accion === 'activar' ? 'activado' : 'anulado'} correctamente` })
          } catch (err) {
             console.error('Error cambio estado cargo:', err) 
