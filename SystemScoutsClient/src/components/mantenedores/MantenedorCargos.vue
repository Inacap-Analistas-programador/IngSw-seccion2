<template>
  <div class="mantenedor-section">
    ...
    <div class="search-bar">
      <input type="text" class="search-input" v-model="search" placeholder="Buscar Cargo...">
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="filteredItems.length === 0">
              <td colspan="3" class="no-data">No se encontraron cargos</td>
            </tr>
          </tbody>
        </table>
      </ModernMainScrollbar>
          ...
          <form @submit.prevent="guardar">
            <div class="form-group">
              ...
              <input type="text" class="form-control" v-model="form.descripcion"
                ...
                placeholder="EJ: JEFE DE GRUPO" required>
            </div>
...
const cargarDatos = async () => {
  ...
  try {
    ...
    items.value = rawData.map(c => ({
      id: c.car_id ?? c.CAR_ID ?? c.id,
      descripcion: (c.car_descripcion ?? c.CAR_DESCRIPCION ?? c.DESCRIPCION ?? c.descripcion ?? '').toString(),
      vigente: !!(c.car_vigente ?? c.CAR_VIGENTE ?? c.vigente ?? true)
    }))
  } catch (error) {
    console.error('Error cargando cargos:', error)
    showToast('Error al cargar cargos', 'alert-triangle')
...
const guardar = async () => {
  saving.value = true
  try {
    const payload = { car_descripcion: form.descripcion, car_vigente: !!form.vigente }
    if (editando.value) {
      if (!form.id) throw new Error('ID no válido')
      await mantenedoresService.cargo.partialUpdate(form.id, payload)
      showToast('Cargo actualizado correctamente', 'check')
    } else {
      await mantenedoresService.cargo.create(payload)
      showToast('Cargo creado correctamente', 'check')
    }
    cerrarModal(); await cargarDatos()
  } catch (error) {
    console.error('Error guardando cargo:', error)
    showToast('Error al guardar cargo', 'alert-triangle')
  } finally { saving.value = false }
}

const confirmarAnular = (item) => {
  emit('confirm-action', {
    titulo: 'Anular Cargo', mensaje: `¿Estás seguro de anular "${item.descripcion}"?`,
    accion: async () => {
      try { await mantenedoresService.cargo.partialUpdate(item.id, { car_vigente: false }); await cargarDatos(); showToast('Cargo anulado correctamente', 'check') }
      catch (e) { showToast('Error al anular cargo', 'alert-triangle') }
    }
  })
}
...
const confirmarActivar = (item) => {
  emit('confirm-action', {
    titulo: 'Activar Cargo', mensaje: `¿Estás seguro de activar "${item.descripcion}"?`,
    accion: async () => {
      try { await mantenedoresService.cargo.partialUpdate(item.id, { car_vigente: true }); await cargarDatos(); showToast('Cargo activado correctamente', 'check') }
      catch (e) { showToast('Error al activar cargo', 'alert-triangle') }
    }
  })
}
