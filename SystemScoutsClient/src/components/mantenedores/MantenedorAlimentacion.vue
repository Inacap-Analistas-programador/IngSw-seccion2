<template>
  <div class="mantenedor-section-expanded">
    ...
    <!-- Modal Crear/Editar -->
    <div v-if="modalActivo" class="modal-overlay" @click="cerrarModal">
          ...
          <form @submit.prevent="guardar">
            <div class="form-group">
              ...
              <input 
                ...
                placeholder="EJ: ALMUERZO NORMAL"
                required
              >
            </div>
  ...
  setup(props, { emit }) {
    ...
    const cargarDatos = async () => {
      ...
      try {
        ...
        items.value = normalize(getData(resp), a => ({
        ...
        console.error('Error cargando alimentacion:', err)
    ...
    const guardar = async () => {
      ...
      try {
        const payload = {
          ali_descripcion: form.descripcion,
          ali_tipo: form.tipo,
          ali_vigente: !!form.vigente
        }
        ...
        cerrarModal()
        await cargarDatos()
        emit('show-message', { type: 'success', text: editando.value ? 'Alimentación actualizada' : 'Alimentación creada' })
      } catch (err) {
    ...
    const confirmarAccion = (item, accion) => {
      emit('confirm-action', {
        ...
        accion: async () => {
          try {
            const payload = { ali_vigente: accion === 'activar' }
            await mantenedoresService.alimentacion.partialUpdate(item.id, payload)
            await cargarDatos()
            emit('show-message', { type: 'success', text: `Registro ${accion === 'activar' ? 'activado' : 'anulado'} correctamente` })
          } catch (err) {
             console.error('Error cambio estado alimentacion:', err) 
